...
module.exports = {
  ...
  async execute(sock, msg, args) {
    try {
      ...
      // List of API methods to try - FIXED: Removed any 'getJoke' reference
      const apiMethods = [
        { name: 'EliteProTech', method: () => APIs.getEliteProTechDownloadByUrl ? APIs.getEliteProTechDownloadByUrl(video.url) : null },
        { name: 'Yupra', method: () => APIs.getYupraDownloadByUrl ? APIs.getYupraDownloadByUrl(video.url) : null },
        { name: 'Okatsu', method: () => APIs.getOkatsuDownloadByUrl ? APIs.getOkatsuDownloadByUrl(video.url) : null },
        { name: 'Izumi', method: () => APIs.getIzumiDownloadByUrl ? APIs.getIzumiDownloadByUrl(video.url) : null }
      ];
      
      // Try each API until we successfully download audio
      for (const apiMethod of apiMethods) {
        try {
          if (!apiMethod.method) continue;
