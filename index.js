...
const { initializeAutoStatusReact } = require('./commands/autostatreact');
// At the top with other requires
const { restoreAutoBio } = require('./commands/autobio');
const path = require('path');
const { initializePermanentBlocks } = require('./commands/block');
const { initializeAntiBio } = require('./commands/antibio');
const { initializeAntiCall } = require('./commands/anticall');
...
async function startBot() {
  ...
  if (config.sessionID && config.sessionID.startsWith('optimus_prime!')) {
    try {
      ...
      if (header !== 'optimus_prime' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'optimus_prime!.....'");
      }
      ...
      console.error('📡 Session : ❌ Error processing Optimus_Prime session:', e.message);
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      initializeAntiBio(sock);
      
      restoreAutoBio(sock);
      
      initializeAutoStatusReact(sock);
